...
export default function CourseEdit() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const [course, deptList] = await Promise.all([
          getCourse(Number(id)),
          getDepartments(),
        ]);
        ...
        // 404 handled by httpClient interceptor (navigates to not-found)
      } finally {
      ...
      <Box component="form" onSubmit={handleSubmit} noValidate>
        ...
        <Box sx={{ mt: 2, mb: 1 }}>
          <Typography variant="body2" color="textSecondary" gutterBottom>
            Teaching Material Image (optional — upload to replace current)
          </Typography>
          <input
            type="file"
